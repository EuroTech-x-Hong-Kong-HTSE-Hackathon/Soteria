...
Use your tools to confirm before deciding:
  - start_verification_timer(): begin a grace period so the person can get up \
    or cancel a false alarm. The result reports whether motion/recovery was \
...
Decision guidance:
  - A single noisy detection is not proof. Confirm over time: start the \
    verification timer first and watch the result for recovery vs. continued \
    stillness.
  - Escalate only if the timer elapses without signs of recovery, or if the \
    detection evidence is strongly conclusive on its own.
